...
from tracking_ros.utils.painter import mask_painter, point_drawer, bbox_drawer, bbox_drawer_with_text
...
def main(args):
    ...
    for i, original_image in enumerate(original_images):
        ...
        decomposed_mask = decompose_mask(template_mask, num_objects=num_objects) # [N, H, W]

        painted_image = original_image.copy()
        for i, mask in enumerate(decomposed_mask):
            painted_image = mask_painter(painted_image, mask.astype(np.bool_), color_index=i*3+1, alpha=0.7)
    ...
    for original_image, masked_painted_image_with_bbox in zip(original_images, masked_painted_images_with_bbox):
        ...
        concat_image = np.concatenate([original_image, masked_painted_image_with_bbox], axis=0)
    ...
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    video = cv2.VideoWriter("masked_painted_with_bbox.mp4", fourcc, 30.0, (width, height))
