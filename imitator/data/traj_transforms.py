"""
Contains trajectory transforms used in the imitator data pipeline. Trajectory transforms operate on a dictionary
...
def pad_actions_and_state(
    traj: dict, max_action_dim: Optional[int], max_state_dim: Optional[int]
) -> dict:
    """Pads actions and states to a maximum number of dimensions across all datasets.
    ...
    if max_state_dim is not None and "state" in traj["observation"]:
        state_dim = traj["observation"]["state"].shape[-1]
        if state_dim > max_state_dim:
            raise ValueError(
                f"state_dim ({state_dim}) is greater than max_state_dim ({max_state_dim})"
            )
        traj["observation"]["state"] = tf.pad(
            traj["observation"]["state"], [[0, 0], [0, max_state_dim - state_dim]]
        )
    return traj
