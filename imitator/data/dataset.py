...
from imitator.data import obs_transforms, traj_transforms
from imitator.data.utils import goal_relabeling, task_augmentation
from imitator.data.utils.data_utils import (
    allocate_threads,
...
from imitator.utils.spec import ModuleSpec
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    Args:
        ...
        max_state_dim (int, optional): If provided, datasets with a state dimension less than this will be
            padded to this dimension.
    ...
    # marks whiche entires of the observation and task dicts are padding
    dataset = dataset.traj_map(traj_transforms.add_pad_mask_dict, num_parallel_calls)

    # optionally pads actions and state to a consistent number of dimensions
    dataset = dataset.traj_map(
        partial(
            traj_transforms.pad_actions_and_state,
    ...
    *,
    ...
    Args:
        ...
        filter_functions (Sequence[ModuleSpec]): ModuleSpecs for filtering functions applied to the
            raw dataset.
        skip_norm (bool): If true, skips normalization of actions and states. Default: False.
    ...
    def restructure(traj):
        ...
        # extracts images, depth images and states from the "observation" dict
        ...
        if state_obs_keys:
            new_obs["state"] = tf.concat(
                [
                    tf.cast(old_obs[state_key], tf.float32)
                    for state_key in state_obs_keys
                ],
                axis=-1,
            )
