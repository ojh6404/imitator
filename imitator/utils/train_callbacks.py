...
from imitator.utils.train_utils import batched_apply, TrainState
from imitator.utils.typing import Any, Data, Sequence
from imitator.utils.visualization_lib import RolloutVisualizer, Visualizer
...
class RolloutVisualizationCallback(Callback):
    ...
    def __post_init__(self):
        ...
        self.rollout_visualizers = [
            RolloutVisualizer(
                action_state_metadata=self.action_state_metadata,
                **kwargs,
            )
        ...
        modal_policy_fns = {
            mode: supply_rng(
                partial(
                    get_policy_sampled_actions,
                    train_state,
                    unnormalization_statistics=self.action_state_metadata["action"],
                    zero_text=self.zero_text,
                    samples_per_state=1,
                    policy_mode=mode,
                )
