...
def run_policy_on_trajectory(policy_fn, traj, *, text_processor=None):
    ...
    return {
        ...
        **(
            {"state": traj["observation"]["state"][:, horizon - 1]}
...
class Visualizer:
    ...
    def __post_init__(self):
        self.action_state_stats = self.dataset.dataset_statistics
    ...
    def visualize_for_wandb(
        ...
        for n, traj in tqdm.tqdm(enumerate(iterator), total=max_trajs):
            ...
            info = add_unnormalized_info(info, self.action_state_stats)
            info = add_manipulation_metrics(info)

            if "unnorm_state" in info:
...
class RolloutVisualizer:
    ...
    Args:
        ...
        action_state_metadata (dict): Dictionary of normalization statistics for state and actions.
    """
    ...
    action_state_metadata: Optional[dict] = None

    def __post_init__(self):
        self._env = gym.make(self.env_name, **self.env_kwargs)
        if self.action_state_metadata is not None:
            self._env = NormalizeState(self._env, self.action_state_metadata)
    ...
    info.update(
        {
            ...
            **(
                {
                    "unnorm_state": unnormalize(
                        info["state"], **normalization_stats["state"]
                    )
                }
                if "state" in info
                else {}
...
def add_manipulation_metrics(info):
    ...
    def per_sample_info(multi_info, single_info):
        ...
        return {
            ...
            **(_gripping_early_metrics(**kwargs) if "state" in kwargs else {}),
        }
...
def plot_trajectory_actions(
    ...
    **kwargs,
    ...
    pred_actions, actions, proprio = unnorm_pred_actions, unnorm_actions, unnorm_state
...
def _gripping_early_metrics(
    unnorm_actions, unnorm_state, unnorm_pred_actions, **kwargs
):
    gripper_closed = _gripper_closed(unnorm_actions)
    pred_gripper_closed = _gripper_closed(unnorm_pred_actions)

    unnorm_state = unnorm_state[:, 1:]  # Remove special dimension
    z_position = unnorm_state[:, 2]
